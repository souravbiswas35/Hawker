...
export default function AdminWomenMentorshipApplicationsPage() {
  ...
  const loadApplications = async () => {
    try {
      ...
      const res = await api.get("/admin/women-support/mentorship-applications", { params });
  ...
  const handleReview = async (applicationId, reviewAction) => {
    try {
      await api.patch(`/admin/women-support/mentorship-applications/${applicationId}/review`, {
        action: reviewAction,
        remarks,
      });
      alert(`Mentorship request ${reviewAction === "accept" ? "accepted" : reviewAction === "reject" ? "rejected" : "marked as completed"} successfully`);
      setShowModal(false);
      setRemarks("");
      loadApplications();
    } catch (err) {
    ...
    <div className="container py-4">
      <div className="d-flex justify-content-between align-items-center mb-4">
        <div>
          ...
          <p className="text-muted mb-0">Women Vendor Support - Manage mentorship requests</p>
        </div>
                    ...
                    <tr key={app.id}>
                      <td>
                        <div>
                          ...
                          {app.business_name && <><br /><small className="text-muted">{app.business_name}</small></>}
                        </div>
                      </td>
            ...
            <div className="modal-content">
              <div className="modal-header">
                <h5 className="modal-title">
                  {action === "accept" ? "Accept Mentorship Request" : action === "reject" ? "Reject Mentorship Request" : "Mark as Completed"}
                </h5>
                <button
                  type="button"
                  className="btn-close"
              ...
              <div className="modal-body">
                <div className="mb-3">
                  ...
                  {selectedApplication.business_name && <p><strong>Business:</strong> {selectedApplication.business_name}</p>}
                </div>
                <div className="mb-3">
                  <h6 className="fw-bold">Mentor Information</h6>
                  <p><strong>Name:</strong> {selectedApplication.mentor_name}</p>
              ...
              <div className="modal-footer">
                ...
                >
                  {action === "accept" ? "Accept" : action === "reject" ? "Reject" : "Mark Complete"}
                </button>
              </div>
            </div>
          </div>
