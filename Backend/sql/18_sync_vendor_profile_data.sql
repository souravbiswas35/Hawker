...
SET
  la.business_name = COALESCE(la.business_name, vp.business_name, CONCAT(vp.first_name, ' ', vp.last_name)),
  la.business_category = COALESCE(la.business_category, vp.business_type, 'General'),
  la.stall_type = COALESCE(la.stall_type, 'Standard'),
  la.desired_zone = COALESCE(
    NULLIF(la.desired_zone, ''),
    NULLIF(la.desired_zone, 'To be selected'),
    NULLIF(vp.vending_zone, ''),
    NULLIF(vp.vending_zone, 'To be selected'),
    'Not assigned'
  )
WHERE la.business_name IS NULL
  OR la.business_name = ''
...
SET 
  vl.current_zone = COALESCE(
    ...
    NULLIF(vl.current_zone, 'To be selected'),
    NULLIF(vp.vending_zone, ''),
    NULLIF(vp.vending_zone, 'To be selected'),
    'Not assigned'
  )
WHERE vl.current_zone IS NULL 
  OR vl.current_zone = '' 
...
SET goods_authorized = COALESCE(
  NULLIF(goods_authorized, ''),
  NULLIF(goods_authorized, 'To be selected'),
  JSON_UNQUOTE(JSON_EXTRACT(business_details, '$.goods_authorized')),
  JSON_UNQUOTE(JSON_EXTRACT(business_details, '$.goods')),
  business_category,
  'General'
)
WHERE status = 'approved' 
  AND (goods_authorized IS NULL OR goods_authorized = '' OR goods_authorized = 'To be selected');
