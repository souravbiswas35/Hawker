...
-- BEFORE: Data integrity verification
...
DELETE FROM vendor_notifications WHERE id IS NULL OR id = 0;
...
SELECT
    ...
    SUM(
        CASE
            ...
            ELSE 0
        END
    ) as valid_notifications,
    SUM(
        CASE
            WHEN user_id IS NOT NULL
            AND user_id != 0 THEN 1
            ELSE 0
        END
    ) as notifications_with_valid_user
FROM vendor_notifications;